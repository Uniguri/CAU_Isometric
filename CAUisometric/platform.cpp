			...
			if (base[level][i][j] == 1) {
				for (int k = 1; k <= CHUNK_SIZE; k++) {
					for (int l = 1; l <= CHUNK_SIZE; l++) {
						...
						locateObject(obj, scene, j * l * TILE_SIZE * scale / 2, i * k * TILE_SIZE * scale / 2);
...
int randomCreatePlatform(int base[MAX_LEVEL][baseN + 1][baseM + 1], int level, int height, int width, int ck) {
	...
	while (cnt-- && ck <= blockLimit) {
		...
		if (ck <= blockLimit && 1 <= dn && dn < baseN && 1 <= dm && dm < baseM && base[level][dn][dm] == 0) {
			ck = randomCreatePlatform(base, level, dn, dm, ck);
		}
		i++;
		if (i == 4)
			i = 0;
